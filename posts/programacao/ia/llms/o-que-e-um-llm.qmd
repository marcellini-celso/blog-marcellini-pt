...
format:
  html:
    ...
    code-link: true
page-layout: full
categories: [inteligência artificial, IA, LLM, machine learning, deep learning, modelos de linguagem, GPT, Quarto, redes neurais, Transformer, NLP, IA generativa, BERT, RNN, LSTM, GRU]
tags: [inteligência artificial, IA, LLM, machine learning, deep learning, modelos de linguagem, GPT, Quarto, redes neurais, Transformer, NLP, IA generativa, BERT, RNN, LSTM, GRU]
---
...
- **GPT (OpenAI)**  
  - Linha de modelos mais popular, com versões como **GPT-3.5** e **GPT-4**.  
...
|--------|---------|--------|----------------|------------|
| **GPT (3.5/4)** | OpenAI | API paga / ChatGPT | Versatilidade geral | Muito usado no mercado |
...
![](images/llms-timeline.png){fig-align="center" width="100%"}
...
outfile = "images/llms-timeline.png"

# dados principais (ano de lançamento)
models = [
    ("GPT-3", 2020, "OpenAI"),
    ("GPT-3.5", 2022, "OpenAI"),
    ("ChatGPT", 2022, "OpenAI"),
...
# desenha pontos
for (lab, year, org) in models:
    ax.scatter(year, org, s=2000, c=colors[org], edgecolor="black", zorder=3)
    ax.text(year, org, lab, ha="center", va="center", fontsize=8, weight="bold")

# ajustes do gráfico
ax.set_title("Linha do Tempo — Modelos GPT, Claude, LLaMA e Mistral", fontsize=12, weight="bold")
ax.set_xlabel("Ano de lançamento")
ax.set_yticks(["OpenAI","Anthropic","Meta","Mistral"])
ax.set_yticklabels(["OpenAI (GPT)", "Anthropic (Claude)", "Meta (LLaMA)", "Mistral (França)"])
